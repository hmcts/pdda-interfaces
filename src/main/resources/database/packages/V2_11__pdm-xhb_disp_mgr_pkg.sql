...
CREATE OR REPLACE FUNCTION xhb_disp_mgr_pkg.get_ip_host_number (pi_ip_address pdda.xhb_disp_mgr_cdu.ip_address%TYPE) RETURNS bigint AS $body$
BEGIN
...
CREATE OR REPLACE FUNCTION xhb_disp_mgr_pkg.host_exists_yn (pi_court_site_id pdda.xhb_disp_mgr_cdu.court_site_id%TYPE, pi_host bigint) RETURNS char AS $body$
DECLARE

	l_exists_YN char(1);
	c_host_check CURSOR FOR
		SELECT 'Y'
		FROM pdda.xhb_disp_mgr_cdu cdu
...
CREATE OR REPLACE PROCEDURE xhb_disp_mgr_pkg.get_next_ip_host (pi_court_site_id pdda.xhb_disp_mgr_cdu.court_site_id%TYPE, pi_min bigint, pi_max bigint, po_next_host INOUT bigint) AS $body$
DECLARE
	...
	c_next_host CURSOR FOR
		...
		FROM (	SELECT xhb_disp_mgr_pkg.get_ip_host_number(cdu.ip_address)+1 next_host
				FROM pdda.xhb_disp_mgr_cdu cdu
				WHERE cdu.court_site_id = pi_court_site_id
				AND NOT EXISTS (SELECT 1 FROM pdda.xhb_disp_mgr_cdu nxt_cdu
