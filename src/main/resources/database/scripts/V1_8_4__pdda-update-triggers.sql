...
BEGIN
	...
	IF NOT EXISTS (
		SELECT 1 FROM pdda.AUD_CPP_STAGING_INBOUND
    		WHERE cpp_staging_inbound_id = OLD.cpp_staging_inbound_id
...
BEGIN
    ...
    IF (XHB_CUSTOM_PKG.IS_AUDIT_REQUIRED('XHB_PDDA_MESSAGE') = 1) THEN
        IF NOT EXISTS (
                ...
                WHERE pdda_message_id = OLD.pdda_message_id
                AND last_update_date = OLD.last_update_date
        ) THEN  
...
BEGIN
    ...
    IF (XHB_CUSTOM_PKG.IS_AUDIT_REQUIRED('XHB_DISPLAY_STORE') = 1) THEN
        IF NOT EXISTS (
                SELECT 1 FROM pdda.AUD_DISPLAY_STORE
                WHERE display_store_id = OLD.display_store_id
                AND last_update_date = OLD.last_update_date
        ) THEN  
...
BEGIN
    ...
    IF (XHB_CUSTOM_PKG.IS_AUDIT_REQUIRED('XHB_CR_LIVE_DISPLAY') = 1) THEN
        IF NOT EXISTS (
                SELECT 1 FROM pdda.AUD_CR_LIVE_DISPLAY
                WHERE cr_live_display_id = OLD.cr_live_display_id
                AND last_update_date = OLD.last_update_date
        ) THEN  
