...
public class MaskingTranslator extends Translator {
     ...
     * 
     * @param parent Parent translator
     */
    ...
    /**
     * This will reset the defendants masked name and masked flag. This because the application is a
     * multi-user app and this causes probelms when/if two or more users are updating the same case
     * at the same time and if one of them is changing the masked details. Therefore we have to make
     * sure this is reset so that no confidential details are being displayed on the Internet page.
     * 
     * @param dom dom
     * @param maskedName masked name
     */
    private void resetDefendantDetails(final Document dom, final String maskedName,
        final String maskedFlag) {
     ...
     * 
     * @param doc Document that will be changed
     * @param tagname String the element by tag name
     * @param value String the value that the document will be updated with
     */
