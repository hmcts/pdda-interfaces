...
public class XhbCourtelListRepository extends AbstractRepository<XhbCourtelListDao>
    ...
    @SuppressWarnings("unchecked")
    public List<XhbCourtelListDao> findCourtelListSafe(final Integer courtelMaxRetry,
        final Integer intervalSeconds, final Integer courtelListAmount) {
        LOG.debug("findCourtelList()");
        
        try (EntityManager em = EntityManagerUtil.getEntityManager()) {
            Query query = em.createNamedQuery("XHB_COURTEL_LIST.findCourtelList");
            ...
            LOG.error("Error in findCourtelList({} {} {}): {}", courtelMaxRetry, intervalSeconds,
                courtelListAmount, e.getMessage(), e);
            return List.of(); // Safe fallback
        }
    }
    
    public Optional<XhbCourtelListDao> findByXmlDocumentClobIdSafe(final Long xmlDocumentClobId) {
        LOG.debug("findByXmlDocumentClobId()");
        
        try (EntityManager em = EntityManagerUtil.getEntityManager()) {
            Query query = em.createNamedQuery("XHB_COURTEL_LIST.findByXmlDocumentClobId");
            query.setParameter("xmlDocumentClobId", xmlDocumentClobId);
            @SuppressWarnings("unchecked")
            List<XhbCourtelListDao> resultList = query.getResultList();
            return resultList.isEmpty() ? Optional.empty() : Optional.of(resultList.get(0));
        } catch (Exception e) {
            LOG.error("Error in findCourtelList({}): {}", xmlDocumentClobId, e.getMessage(), e);
