...
class XhbCourtelListRepositoryTest extends AbstractRepositoryTest<XhbCourtelListDao> {
    ...
    private boolean testFindBy(String query, List<XhbCourtelListDao> list) {
        // Ensure repo is constructed
        getClassUnderTest();

        // Common stubs for Query
        Mockito.when(mockQuery.setParameter(Mockito.anyString(), Mockito.any()))
               .thenReturn(mockQuery);
        Mockito.when(mockQuery.getResultList()).thenReturn(list);

        if (QUERY_XMLDOCUMENTID.equals(query)) {
            // This path uses the injected EntityManager (no static mocking needed)
            ...
            Optional<XhbCourtelListDao> result = getClassUnderTest().findByXmlDocumentId(getDummyId());
            assertNotNull(result, NOTNULL);
            if (list.isEmpty()) {
                assertSame(Optional.empty(), result, SAME);
            } else {
            ...
            // This path uses EntityManagerUtil.getEntityManager() -> static mock it
            try (MockedStatic<EntityManagerUtil> emUtil = Mockito.mockStatic(EntityManagerUtil.class)) {
                emUtil.when(EntityManagerUtil::getEntityManager).thenReturn(mockEntityManager);
                Mockito.when(mockEntityManager.createNamedQuery("XHB_COURTEL_LIST.findCourtelList"))
                       .thenReturn(mockQuery);

                List<XhbCourtelListDao> resultList =
                    getClassUnderTest().findCourtelListSafe(DUMMY_COURTEL_MAX_RETRY,
                                                            DUMMY_INTERVAL,
                ...
                if (list.isEmpty()) {
                    ...
                    // expect 'sixMinutesAgo' and 'never' only
            ...
            // Also uses EntityManagerUtil.getEntityManager()
            try (MockedStatic<EntityManagerUtil> emUtil = Mockito.mockStatic(EntityManagerUtil.class)) {
                emUtil.when(EntityManagerUtil::getEntityManager).thenReturn(mockEntityManager);
                ...
                Optional<XhbCourtelListDao> result =
                    getClassUnderTest().findByXmlDocumentClobIdSafe(getDummyLongId());
                assertNotNull(result, NOTNULL);
                if (list.isEmpty()) {
                    assertSame(Optional.empty(), result, SAME);
                } else {
